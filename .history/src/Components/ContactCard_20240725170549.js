...
const ContactCard = (props) => {
    const { contact, setContact, setEditContact } = props;

    const clickHandler = (item) => {
        setEditContact(item); // Assuming you want to edit this contact
        console.log("Editing contact:", item);
    };
                    ...
                    {contact && contact.map((item) => (
                        <tr key={item.id}>
                            ...
                            <td className='align-content-center' data-label="Action">
                                ...
                                <Link to="/edit">
                                    <i className="bi bi-pencil-square" style={{ color: "green", marginTop: "7px", marginLeft: "7px" }} onClick={() => clickHandler(item)}></i>
                                </Link>
                            </td>
                        </tr>
                    ))}
